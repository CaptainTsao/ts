...
int File::SafeRead(int fd, char *buf, size_t size) {
  int offset = 0;
  int char_read = 0;
  do {
    char_read = (int) TEMP_FAILURE_RETRY(read(fd, buf, size));
    if (char_read < 0) {
...
ssize_t File::SafeWrite(int fd, char *buf, size_t size) {
  int char_written;
  size_t offset = 0;
  do {
    char_written = (int) TEMP_FAILURE_RETRY(write(fd, buf, size));
    if (char_written < 0) {
...
ssize_t File::SafeWriteLine(int fd, char *buf, size_t size) {
  ssize_t char_written = SafeWrite(fd, buf, size);
...
int File::CreateDir(char *dirName) {
  ...
  if (-1 == mkdir(dirName, mode)) {
    fprintf(stderr, "Error occurred when creating directory: %s\n Error code = %d\n", dirName, errno);
    if (errno == EEXIST) {
...
int File::CreateDirIfNotExists(char *dirName) {
...
int File::CreateDirInPath(char *dirName, char *path) const {
  int return_value = 0;
  char *current_dir = GetCurrentPath();
  if (current_dir) {
    return_value = CreateDirInPathAndBackToCurrentDir(dirName, path, current_dir);
    free(current_dir);
    return return_value;
  }
  return -1;
...
char *File::AllocCombinedPath(const char *path1, const char *path2) {
  size_t size = 1;
...
void File::CombinePaths(char *destination, const char *path1, const char *path2) {
  if (path1 && *path1) {
...
FILE *File::OpenStream(char *fileName, char *mode) {
...
int File::CloseStream(FILE *file) {
  if (0 != fclose(file)) {
...
struct dirent *File::GetFileDirent(char *path, char *name) const {
  ...
  struct dirent *dp;
  if (nullptr == dirp) return nullptr;
  do {
    errno = 0;
    if ((dp = ReadDirectory(dirp)) != nullptr) {
